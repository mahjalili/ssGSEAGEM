function tissueModel = solve_INIT(model, weights, tol, core, logfile, runtime)
...
if nargin < 6 || isempty(runtime)
    runtime = 7200;
end
if nargin < 5 || isempty(logfile)
    logfile = 'MILPlog';
end
if nargin < 4 || isempty(core)
    core={};
end
if nargin < 3 || isempty(tol)
    ...
    solution = solveCobraMILP(MILPproblem, 'timeLimit', runtime, 'logFile', logfile, 'printLevel', 0);

    x = solution.cont;
    %rxnRemList = model.rxns(abs(x) < tol);
    %FIX
    rxnRemList = setdiff(model.rxns(abs(x) < tol), core);
    %FIX
