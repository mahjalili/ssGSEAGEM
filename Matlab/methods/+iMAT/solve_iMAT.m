function [tissueModel] = solve_iMAT(model, expressionRxns, threshold_lb, threshold_ub, tol, core, logfile, runtime)
...
if nargin < 5 || isempty(core)
    ...
    solution = solveCobraMILP(MILPproblem, 'timeLimit', runtime, 'logFile', logfile, 'printLevel', 0, 'saveInput', '');
    
    solution.x = solution.cont;
    solution.f = solution.obj;
    %rxnRemList = model.rxns(abs(solution.x) < tol);
    %FIX
    rxnRemList = setdiff(model.rxns(abs(solution.x) < tol), core);
    %FIX
