...
\nError message returned by 'cmapR::parse.gctx()':\n\n", dataset, '\n\n')
    ...
    #################################################
    ## score matrix
    V.mat <- data.matrix(score.matrix.2)

    #################################################
    ## p-value matrix
    P.mat <- data.matrix(pval.matrix.2)
    
    ## ##############################################  
    ## FDR matrix
    F.mat <- data.matrix(fdr.matrix.2) #F.GCT.mat
    ...
    score.tmp <- V.mat 
    colnames(score.tmp) <- paste('score', sample.names, sep='.')
    gs.descs.2 <-  data.frame(score.tmp, pval.tmp, fdr.tmp, stringsAsFactors = F)
    row.names(gs.descs.2) <- gs.names.2
    
    fn <- paste (output.prefix, '-combined.csv', sep='')
	write.table(gs.descs.2, fn, append = FALSE, sep = ",", dec = ".", row.names = TRUE, col.names = TRUE)	
